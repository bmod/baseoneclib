...
void TimeDisplay::setHatchSpacing(qreal minor, qreal major) {
    mMinorHatchSpacing = minor;
    mMajorHatchSpacing = major;
}

qreal SMPTETimeDisplay::calcStepInterval(qreal windowSize,
                                         qreal viewSize,
...
const QString SMPTETimeDisplay::timeToString(qreal interval, qreal time) const {
...
SMPTETimeDisplay::SMPTETimeDisplay(int framerate) {
    setFramerate(framerate);
}

void SMPTETimeDisplay::setFramerate(int framerate) {
    mFramerate = framerate;
}

qreal GeneralTimeDisplay::calcStepInterval(qreal windowSize,
                                           qreal viewSize,
                                           qreal minStepSize) const {
    qreal ival;
...
const QString GeneralTimeDisplay::timeToString(qreal interval, qreal time) const {
    if (time == 0)
        return "0";
...
qreal FloatTimeDisplay::calcStepInterval(qreal windowSize, qreal viewWidth,
                                         qreal minStepSize) const {
...
const QString FloatTimeDisplay::timeToString(qreal interval, qreal time) const {
    return QString::number(time);
}
