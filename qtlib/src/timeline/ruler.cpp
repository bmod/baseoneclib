...
Ruler::Ruler(QWidget* parent) : QWidget(parent) {
    mTimeConfig = std::make_shared<GeneralTimeDisplay>();
...
void Ruler::setRange(const Range& range) {
    if (mRange == range)
        return;
...
void Ruler::setTimeDisplay(std::shared_ptr<TimeDisplay> display) {
    mTimeConfig = display;
    update();
}
...
void Ruler::drawHatches(QPainter& painter, int hatchLength, qreal minStepSize, bool drawLabels) const {
...
void Ruler::setHeight(int height) {
    setMinimumHeight(height);
    setMaximumHeight(height);
}

void Ruler::setHatchFont(const QFont& font) {
    mFont = font;
